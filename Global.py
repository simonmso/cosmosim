...
class ConstantsAndUnits:
    ...
    def __init__(
        ...
        if typeOfUnits == "SI":
            ...
            self.m = (
                np.sqrt((self._c_SI**3) / (self._hbar_SI * self._G_SI)) / factor**0.5
            )
        ...
        self.H0_over_h = 100.0 * self.km / self.s / self.Mpc  # H0 / h
        self.epsilon_0 = (
            self._epsilon_0_eV * self.eV
        )  # Ionization energy for the ground state of hydrogen
        self.xhi0 = self._xhi0_eV * self.eV  # Ionization energy for neutral Helium
        self.xhi1 = (
            self._xhi1_eV * self.eV
        )  # Ionization energy for singly ionized Helium
